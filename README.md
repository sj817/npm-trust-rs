...
`npt provision` (also the menu's _batch placeholder publish_ entry) covers the other batch shape — names that do not exist yet. Nothing has to exist locally either: no package directory, no checkout, no `package.json`. Every placeholder is generated in a temp directory, published, and removed. Tick the primary package and/or the platform packages, give it the primary name (or, for platform-only runs, a bare prefix), then choose how sub-package names are derived (`@scope/native-<os>-<arch>` by default) and which `<os>-<arch>` vocabulary to use (`win32-x64` / `windows-amd64` / `win-amd64`). It reserves every name with a 0.0.1 placeholder and binds them all to the same repository, one OTP for the run. `--dry-run` prints the name list and stops.

`npt provision`（也就是菜单里的「批量占位发布 + 绑定仓库」）处理另一种批量场景——包名还不存在。本地也不需要有任何东西：没有包目录、没有 clone、没有 `package.json` 都可以，每个占位包都在临时目录里生成，发布完即删。勾选主包和 / 或跨平台包，输入主包名（只发跨平台包时则输入一个前缀），再选择子包命名方式（默认 `@scope/native-<os>-<arch>`）和 `<os>-<arch>` 词表（`win32-x64` / `windows-amd64` / `win-amd64`）。它会为每个名字发一个 0.0.1 占位版并统一绑定到同一个仓库，整批只需一次 OTP；`--dry-run` 只打印包名清单。
